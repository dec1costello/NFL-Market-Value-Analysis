...
class Matchup:
    ...
    def __repr__(self):
        return f"Matchup({self.player_id} vs {self.opponent_id}: {self.base_metric:.3f})"
...
class WRModel(PositionModel):
    ...
    def compute_base_metric(self, game_stats) -> float:
        ...
        if game_stats['targets'] > 0:
            ...
            print(f"    Computing WR metric: {game_stats['epa']:.2f} EPA / {game_stats['targets']} targets = {epa_per_target:.3f}")
...
class RBModel(PositionModel):
    ...
    def compute_base_metric(self, game_stats) -> float:
        ...
        print(f"    Computing RB metric: {game_stats['epa']:.2f} EPA / {game_stats['carries']} carries = {epa_per_carry:.3f}")
...
class MutualOpponentModel:
    ...
    def fit(self, matchups: List[Matchup], max_iter: int = 100, tol: float = 1e-4):
        ...
        for iteration in range(max_iter):
            ...
            for pid, obs_list in player_obs.items():
                ...
                if effective_samples > 0:
                    ...
                    shrunk_rating = (raw_rating * effective_samples) / (effective_samples + k)
                    self.player_ratings[pid] = shrunk_rating
                    
                    if iteration == 0 and pid in list(player_obs.keys())[:2]:  # Show first couple
                        print(f"      {pid}: raw={raw_rating:.3f}, shrunk={shrunk_rating:.3f}, samples={effective_samples:.1f}")
            ...
            for oid, obs_list in opponent_obs.items():
                ...
                if effective_samples > 0:
                    ...
                    raw_rating = total_residual / effective_samples
                    shrunk_rating = (raw_rating * effective_samples) / (effective_samples + k_opponent)
                    self.opponent_ratings[oid] = shrunk_rating
                    
                    if iteration == 0 and oid in list(opponent_obs.keys())[:2]:
                        print(f"      {oid}: raw={raw_rating:.3f}, shrunk={shrunk_rating:.3f}, samples={effective_samples:.1f}")
            
            # Center player ratings (mean = 0)
            print(f"   c. Centering ratings...")
            avg_player = sum(self.player_ratings.values()) / len(self.player_ratings)
            self.player_ratings = {k: v - avg_player for k, v in self.player_ratings.items()}
            
            # Adjust opponents accordingly
            self.opponent_ratings = {k: v + avg_player for k, v in self.opponent_ratings.items()}
            ...
            print(f"   d. Max change: players={player_change:.6f}, opponents={opponent_change:.6f}")
        ...
        print(f"Final player ratings range: [{min(self.player_ratings.values()):.3f}, {max(self.player_ratings.values()):.3f}]")
        print(f"Final opponent ratings range: [{min(self.opponent_ratings.values()):.3f}, {max(self.opponent_ratings.values()):.3f}]")
    
    def predict(self, player_id: str, opponent_id: str) -> float:
        """Predict performance for a matchup"""
        prediction = self.league_avg + self.player_ratings.get(player_id, 0.0) - \
        ...
        for m in matchups:
            try:
                ...
                if m.player_id in ['WR1', 'WR2', 'RB1']:  # Show examples
                    print(f"  {m.player_id} Week {week}: original={original_weight:.3f}, recency={recency_weight:.3f}, final={m.weight:.3f}")
            except:
        ...
        if self.quality_weight:
            ...
            for m in matchups:
                ...
                if m.player_id in ['WR1', 'WR2']:  # Show examples
                    print(f"  {m.player_id} vs {m.opponent_id}: strength={opponent_strength:.3f}, multiplier={quality_multiplier:.3f}, weight {original_weight:.3f}→{m.weight:.3f}")
            
            print("\n3. Re-fitting with quality-weighted data...")
            super().fit(matchups)
    
    def get_confidence_interval(self, player_id: str, alpha: float = 0.05) -> Tuple[float, float]:
...
def create_sample_data():
    ...
    wr_data = [
        {'wr_id': 'WR1', 'defense_id': 'DEF1', 'game_id': '2023_1', 'epa': 12.5, 'targets': 8, 'routes': 40},
        {'wr_id': 'WR1', 'defense_id': 'DEF2', 'game_id': '2023_2', 'epa': 8.2, 'targets': 6, 'routes': 38},
        {'wr_id': 'WR2', 'defense_id': 'DEF1', 'game_id': '2023_1', 'epa': 6.8, 'targets': 5, 'routes': 35},
        {'wr_id': 'WR2', 'defense_id': 'DEF3', 'game_id': '2023_3', 'epa': 15.3, 'targets': 9, 'routes': 42},
        {'wr_id': 'WR3', 'defense_id': 'DEF2', 'game_id': '2023_2', 'epa': 4.5, 'targets': 4, 'routes': 30},
        {'wr_id': 'WR3', 'defense_id': 'DEF3', 'game_id': '2023_3', 'epa': 9.6, 'targets': 7, 'routes': 36},
    ]
    
    # Sample RB data
    rb_data = [
        {'rb_id': 'RB1', 'run_defense_id': 'RDEF1', 'game_id': '2023_1', 'epa': 8.5, 'carries': 15},
        {'rb_id': 'RB1', 'run_defense_id': 'RDEF2', 'game_id': '2023_2', 'epa': 12.3, 'carries': 18},
        {'rb_id': 'RB2', 'run_defense_id': 'RDEF1', 'game_id': '2023_1', 'epa': 5.2, 'carries': 12},
        {'rb_id': 'RB2', 'run_defense_id': 'RDEF3', 'game_id': '2023_3', 'epa': 9.8, 'carries': 16},
    ]
